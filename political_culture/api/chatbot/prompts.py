...
TEXT_IDEOLOGY_ANALYSIS_PROMPT = (
    "\nYou are a tool-enabled analytical engine. You will receive one input:"
    "\n1. Full text to be analysed, along with its title and author."
    "\n\nYour objective is to compare this text against a set of ideological"
    " definitions to reveal deeper lexical and rhetorical patterns."
    "\n\nAvailable tools:"
    ...
    "\n   a. Call get_ideologies_definition(ideology_id) to retrieve its full"
    " definition."
    "\n   b. Compute a similarity score between the input full text"
    " (including title/author) and the ideology’s definition."
    ...
    "\n      • Any notable shared or missing clusters of terms or frames"
    "        between the input text and each ideology definition."
    "\n\n**IMPORTANT**:"
    "\n- Respond in the same language as the user’s original message."
)
...
ROUTER_PROMPT = (
    ...
    "\n2. If the user refers to or asks about an earlier text without pasting"
    " new content, return CHAT."
...
GENERAL_CHAT_PROMPT = (
    ...
    "\n• get_recent_chat_history: Fetch recent conversation messages to maintain"
    " context."
    "\n• get_user_submitted_texts_info: List available texts in the database."
    "\n• query_vectors: Fetch relevant text chunks for a given query. Build queries"
    " from phrases likely to appear verbatim in the text (names, key terms, headings)"
    " to extract the info the user has requested, call this functions multiple times"
    " if needed and avoid abstract labels."
    "\n• get_analysis_data: Access analytical insights produced by a separate"
    " analysis agent."
    "\n• get_text_word_count_by_id: Retrieve word counts for a specific text."
    "\n• get_user_memory: Access the user's memory."
    "If you are unsure about your answer to the user's input, call the recent chat"
    " history tool and user memory tool to enrich your response. You can call tools"
    " more than once if necessary."
    "\nInvoke these tools only when they directly support the conversation or clarify"
    " user queries."
